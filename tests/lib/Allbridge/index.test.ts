...
import { ALLBRIDGE_VERSIONS, AllBridgeData } from './data';

describe('AllBridgeParser', () => {
  let utils: ProtocolParserUtils;

  beforeAll(async () => {
    utils = new ProtocolParserUtils(protocols.allBridge.identifier);
  ...
  it('should parse v1 transactions correctly', async () => {
    const v1Transactions = AllBridgeData[ALLBRIDGE_VERSIONS.V1];
