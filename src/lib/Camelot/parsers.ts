...
export class CamelotParser {
  ...
  public static async parseCamelotRouterTransaction(
  ...
  public static async parseYakRouterTransaction(
    transaction: ITransaction
  ): Promise<ITransactionAction[]> {
    const actions: ITransactionAction[] = [];
    const parsedTxn = ProtocolHelper.parseTransaction(
    ...
    const swapLogs = transaction.logs.filter(
      (log) => log.topics[0] === EVENT_ENUM.YAK_SWAP
    );

    if(!swapLogs) return actions;
    for (const swapLog of swapLogs) {
      const parsedLog = ProtocolHelper.parseLog(
        swapLog,
        contracts.CAMELOT_YOK_ROUTER.events[EVENT_ENUM.YAK_SWAP]
      );
      actions.push(
        {
          type: ACTION_ENUM.SINGLE_SWAP,
          fromToken: parsedLog.args._tokenIn,
          toToken: parsedLog.args._tokenOut,
          fromAmount: parsedLog.args._amountIn.toString(),
          toAmount: parsedLog.args._amountOut.toString(),
  ...
  public static async parseSwapRouterTransaction(
    ...
    const parsedTxn = contracts[contractName].interface.parseTransaction({
  ...
  private static removeDuplicateSwaps(
    swaps: ITransactionAction[]
  ): ITransactionAction[] {
