...
import { IBridgeInAction, ITransaction, ITransactionAction, ITransactionLog } from '../../types';
...
export class DlnSourceContractParseTransaction {
  ...
  public static parseTransaction(transaction: ITransaction): ITransactionAction[] {
    ...
    const matchedPlacedOrderLog = transaction.logs.find((log) => log.topics[0] === EVENT_ENUM.ORDER_PLACED);
  ...
  private static parsePlacedOrder(placeOrderLog: ITransactionLog): IBridgeInAction {
    try {
      ...
      return {
        ...
        fromAmount: (BigInt(order.giveAmount) + BigInt(structuredEvent.percentFee)).toString(),
...
export class DlnDestinationContractParseTransaction {
  ...
  public static parseTransaction(transaction: ITransaction): ITransactionAction[] {
    ...
    const matchedFulfilledOrderLog = transaction.logs.find((log) => log.topics[0] === EVENT_ENUM.ORDER_FULFILLED);
  ...
  private static parseFulfilledOrder(fulfilledOrderLog: ITransactionLog): IBridgeInAction {
    try {
